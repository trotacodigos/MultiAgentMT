...
def get_api(api_keys: List[str], requests: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    with ProcessPoolExecutor(max_workers=params['num_workers']) as executor:
        for idx, request in tqdm(enumerate(requests), total=len(requests), desc="MultiagentMT", ncols=100, colour="yellow"):
...
def get_one_api(api_key: str, request: dict):
    response = call(api_key=api_key, request=request)
    response = verify_response(response)
    ...
    if response['content'] is None:
        ...
        updated['temperature'] += 1
        logging.warning(f"Retrying with increased temperature: {updated['temperature']}")
        response = call(api_key=api_key, request=updated)
        response = verify_response(response)
...
def verify_response(response) -> Dict[str, Any]:
    if hasattr(response, "choices"):
        ...
        if choice.message.content and choice.finish_reason == "stop":
            ...
            tokens = usage_obj.total_tokens if usage_obj and hasattr(usage_obj, "total_tokens") else 0
            return {"content": choice.message.content.strip(), "usage": tokens}
...
def calc_default_params(num_keys: int) -> Dict[str, Any]:
    ...
    return {"num_workers": num_workers, "time_btw_request": time_btw_request}
